// /api/v1/projects/:projectId/lists/:listId/lists
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  const {
    ...
    method,
    query: { projectId: boardId, listId },
    body,
  } = req;
  ...
  const card = { name: body.name, body: body.body };

  switch (method) {
    case "POST":
      try {
        const response = await fetch(`${ZERO_API_URL}/boards/${boardId}/lists/${listId}/cards`, {
          method: "POST",
          headers: {
